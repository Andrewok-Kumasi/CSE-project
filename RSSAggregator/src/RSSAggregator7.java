 ...
 *
 * @author Andrews OSei Kontoh
 *
 */
public final class RSSAggregator7 {

    /**
     * Private constructor so this utility class cannot be instantiated.
     */
    private RSSAggregator7() {
    }
    ...
    private static void outputHeader(XMLTree channel, SimpleWriter out) {
        ...
        int numchildrenTitle = channel.child(titleIndex).numberOfChildren();
        int numChildrenDescription = channel.child(descIndex)
                .numberOfChildren();
        ...
        if (channel.child(titleIndex).numberOfChildren() > 0) {
            out.print("<html> <head> <title>"
                    ...
                    + "</title></head> <body>\r\n" + "      <h1>"
                    + channel.child(titleIndex).child(0).label() + " </h1>\r\n"
                    + "      <p>\r\n");
        } else {
            out.print("<html> <head> <title>" + "No Title"
                    + "</title></head> <body>\r\n" + "      <h1>" + "No Title"
                    + " </h1>\r\n" + "      <p>\r\n");
        }

        if (channel.child(descIndex).numberOfChildren() > 0) {
            out.print(channel.child(descIndex).child(0).label() + "\r\n"
                    + "      </p>\r\n" + "      <table border=\"1\">\r\n"
                    + "      <tr>\r\n" + "      <th>Date</th>\r\n"
                    + "      <th>Source</th>\r\n" + "      <th>News</th>\r\n"
                    + "      </tr>");
        } else {
            out.print("Description is not available" + "\r\n" + "      </p>\r\n"
                    + "      <table border=\"1\">\r\n" + "      <tr>\r\n"
                    + "      <th>Date</th>\r\n" + "      <th>Source</th>\r\n"
                    + "      <th>News</th>\r\n" + "      </tr>");
        }
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        if (index >= 0) {
            out.print("<tr>\n<td>" + item.child(index).child(0).label()
                    + "</td>");
        } else {
            out.print("<td> </td>\n");
        }
        ...
        if (index >= 0 && item.child(index).numberOfChildren() > 0) {
            if (link > 0) {
                out.print("<td><a href=\"" + item.child(link).child(0).label()
            ...
            if (index >= 0 && item.child(link).numberOfChildren() > 0) {
                out.print("<td><a href=\"" + item.child(link).child(0).label()
                        + "\">" + item.child(index) + "</td></tr>");
            } else {
                out.print("<td> NO  Title </td></tr>");
            }
        }
    }
    ...
    /*
     * Creates HTML file for page with list of links to feeds
     *
     * @param xml xmltree for the url the user provides
     *
    ...
    private static void mainPage(XMLTree xml, SimpleWriter out) {
        ...
        for (int a = 0; a < xml.numberOfChildren(); a++) {
            out.print("<li><a href=\"" + xml.child(a).attributeValue("file")
    ...
    public static void main(String[] args) {
        ...
        if (XMLFile.label().contentEquals("feeds")
                || XMLFile.hasAttribute("version") && XMLFile
                        .attributeValue("version").contentEquals("1.0")) {
            //Taking the name of file to generate the HTML page into.
            cout.print(
                    "Enter the name of file to generate the HTML page with Links"
                            + " to other pages: ");
            ...
            cout.print("Not a valid rss url");
        }
        in.close();
        cout.close();
    }
